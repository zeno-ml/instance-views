...
function create_fragment$3(ctx) {
	...
	return {
		...
		h() {
			...
			attr(p, "class", p_class_value = "chat message " + (/*output*/ ctx[0] ? 'model-border' : 'no-model-border') + " svelte-12c99y");
			attr(div, "class", "box svelte-12c99y");
		},
		m(target, anchor) {
			...
			p.innerHTML = /*renderedInput*/ ctx[1];
		},
		p(ctx, [dirty]) {
			...
			if (dirty & /*output*/ 1 && p_class_value !== (p_class_value = "chat message " + (/*output*/ ctx[0] ? 'model-border' : 'no-model-border') + " svelte-12c99y")) {
